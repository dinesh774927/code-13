...
class AppStore extends Component {
  state = {types: tabsList[0].tabId, searchInput: ''}
  ...
  categoryChange = value => {
    this.setState(prev => ({types: value, searchInput: prev.searchInput}))
  }

  render() {
    const {types, searchInput} = this.state
    const newlist = appsList.filter(each => each.category === types)
    const filterSearch = newlist.filter(each =>
